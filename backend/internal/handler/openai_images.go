...
func (h *OpenAIGatewayHandler) Images(c *gin.Context) {
	...
	channelMapping, _ := h.gatewayService.ResolveChannelMappingAndRestrict(requestContext, apiKey.GroupID, requestModel)
	...
	if err := h.billingCacheService.CheckBillingEligibility(requestContext, apiKey.User, apiKey, apiKey.Group, subscription, service.QuotaPlatform(requestContext, apiKey)); err != nil {
	...
	for {
		...
		result, err := func() (*service.OpenAIForwardResult, error) {
			...
			return h.gatewayService.ForwardImages(requestContext, c, account, body, parsed, channelMapping.MappedModel)
		}()
