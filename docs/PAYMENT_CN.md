...
| **易支付主备故障切换（priority-failover）** | 仅对路由到易支付的支付宝、微信支付生效；按服务商列表顺序优先使用主实例，确认创建失败后才尝试后续备份实例 |

使用主备故障切换时，可以启用多个支持同一支付方式的易支付实例。拖拽服务商列表可调整优先级，排在前面的实例为主用，后续实例依次作为备份。单笔或每日限额不足的实例会被跳过。

为避免重复收款，只有本地配置错误、DNS/TLS/连接被拒绝，或易支付明确拒单时才会自动切换；超时、连接中断、响应解析失败等结果不确定的情况会直接报错。参与主备的易支付实例必须使用二维码模式，`popup` 模式在服务端不创建订单，无法可靠故障切换。
